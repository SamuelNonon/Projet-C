...
Tourelle* creer_tourelle(int type, int ligne, int position) {
    ...
    if (type == (int)'T') {     // Tourelle Classique
        t->pointsDeVie = 3;
        t->prix = 100;
        t->degats = 1;
    }
    ...
    if (type == (int)'M') {     // Mine - tue un etudiant à sa destruction
        t->pointsDeVie = 1;
        t->prix = 50;
        t->degats = 0;
    }
    if (type == (int)'S') {     // Sniper
        t->pointsDeVie = 4;
        t->prix = 250;
        t->degats = 3;
    }
    if (type == (int)'P') {     // Perforant - touche les 2 premiers enemis d'une ligne
        t->pointsDeVie = 4;
        t->prix = 200;
        t->degats = 1;
    }
